...
public class VillagerFishingHookRenderer extends EntityRenderer<VillagerFishingHook> {
    ...
    public void render(VillagerFishingHook p_114705_, float p_114706_, float p_114707_, PoseStack p_114708_, MultiBufferSource p_114709_, int p_114710_) {
        Entity owner = p_114705_.getOwner();
        if (owner != null && owner instanceof LivingEntity livingEntity) {
            p_114708_.pushPose();
            p_114708_.pushPose();
            ...
            float f = livingEntity.getAttackAnim(p_114707_);
            float f1 = Mth.sin(Mth.sqrt(f) * (float) Math.PI);
            float f2 = Mth.lerp(p_114707_, livingEntity.yBodyRotO, livingEntity.yBodyRot) * ((float) Math.PI / 180F);
            ...
            if ((this.entityRenderDispatcher.options == null || this.entityRenderDispatcher.options.getCameraType().isFirstPerson()) && owner == Minecraft.getInstance().player) {
                ...
                d4 = Mth.lerp((double) p_114707_, owner.xo, owner.getX()) + vec3.x;
                d5 = Mth.lerp((double) p_114707_, owner.yo, owner.getY()) + vec3.y;
                d6 = Mth.lerp((double) p_114707_, owner.zo, owner.getZ()) + vec3.z;
                f3 = owner.getEyeHeight();
            } else {
                d4 = Mth.lerp((double) p_114707_, owner.xo, owner.getX()) - d1 * d2 - d0 * 0.8D;
                d5 = owner.yo + (double) owner.getEyeHeight() + (owner.getY() - owner.yo) * (double) p_114707_ - 0.45D;
                d6 = Mth.lerp((double) p_114707_, owner.zo, owner.getZ()) - d0 * d2 + d1 * 0.8D;
                f3 = owner.isCrouching() ? -0.1875F : 0.0F;
            }
